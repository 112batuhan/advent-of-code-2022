...
pub fn find_duplicate(playfield: &Vec<Vec<bool>>) -> Option<(usize, usize)> {
    if playfield.len() < 5 {
        return None;
    }

    for cut_start in 0..playfield.len() / 2 {}
...
pub fn result() {
    ...
    for direction in line.chars().cycle() {
        if let None = rock {
            ...
            if number_of_rocks >= 1000000000000 {
                break;
            }
        ...
        //dbg!(direction);
        //debug_field(&playfield, rock.as_ref().unwrap());
        //pause();
